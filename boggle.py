...
from random import choice # single random element from a sequence
import string # common string operations, like string.ascii_uppercase


class Boggle():

    def __init__(self):
        # initialize -- like JS constructor
        self.words = self.read_dict("words.txt")

    def read_dict(self, dict_path):
        """Read and return all words in dictionary."""

        dict_file = open(dict_path) # opens words.txt
        words = [w.strip() for w in dict_file] # new list of all words in words.txt, stripped of leading and trailing spaces
        dict_file.close() # close words.txt
        return words

    # Append 5 rows, each with 5 random letters, to empty board array.
    def make_board(self):
        """Make and return a random boggle board."""
        board = []
        for y in range(5): # range(5) is 0, 1, 2, 3, 4
            row = [choice(string.ascii_uppercase) for i in range(5)] # .ascii_uppercase -- 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'
            board.append(row)

        return board
        # Will be a list matrix w/ random letter assignments:
        # [
        #     ['R', 'P', 'Y', 'P', 'S'],
        #     ['J', 'L', 'Z', 'O', 'U'],
        #     ['C', 'C', 'W', 'M', 'G'],
    ...
    # ONE JOB: check_valid_word() and return 1 of 3 strings.
    # Calls another function, find(), which calls find_from(), and gets complicated.
    # Takes in board and a word, then returns string: "not-on-board", "not-word", or "ok"
    def check_valid_word(self, board, word):
        """Check if a word is a valid word in the dictionary and/or the boggle board"""

        word_exists = word in self.words # T/F: is word in the txt file/words list?
        valid_word = self.find(board, word.upper()) # T/F: is word anywhere on the board?
